...
def get_pins(observed: str) -> Tuple[LiteralString, ...]:
    """
    :param observed: observed PIN
    :return: all possible PINs taking in account all adjacent keys to the ones in observed
    """

    return tuple("".join(combination)
                 for combination in product(*[tuple(key) + ADJACENTS[key]
                                              for key in observed]))
