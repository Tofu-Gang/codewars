...
Part of Series 2/3
This kata is part of a series on the Morse code. Make sure you solve the previous part before you try this one. After
you solve this kata, you may move to the next one.
...
detected on a remote station. The Morse code encodes every character being transmitted as a sequence of "dots" (short
presses on the key) and "dashes" (long presses on the key).
...
def decode_morse(morse_code: str) -> str:
    """
    :param morse_code: string in morse code
    ...
    for letter in letters:
        if letter == "":
            # three spaces between words, insert a space; there will be two spaces between words in the result
            result.append(" ")
        else:
...
def decode_bits(bits: str):
    """
    :param bits: raw signal
    :return: signal decoded to morse code
    """

    # remove zero bits from the beginning and end of the transmission
    while bits.startswith("0"):
        bits = bits[1:]
    while bits.endswith("0"):
        bits = bits[:-1]
    # get signal parts (changes between zero and ones, segments with only zeros or ones)
    signal_parts = [item[0] for item in findall(r"((.)\2*)", bits)]
    # get sample rate, i.e. how many ones is a dot
    sample_rate = min(len(part) for part in signal_parts)
    # normalize the signal (a dot is one one, and so on)
    signal_normalized = "".join(part[:int(len(part) / sample_rate)] for part in signal_parts)
    # decode the raw signal to morse code
    return (signal_normalized.replace('111', '-')
            .replace('000', ' ')
            .replace('1', '.')
            .replace('0', ''))
