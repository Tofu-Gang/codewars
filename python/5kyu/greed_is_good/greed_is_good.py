...
def score_triple(dice):
    """
    :param dice: Greed roll
    :return: score for three same dice
    """

    return (sum(die * 100 if dice.count(die) >= 3 else 0
               for die in range(2, 7))
            + (1000 if dice.count(1) >= 3 else 0))

def score_single(dice):
    """
    :param dice: Greed roll
    :return: score for single (or two) di(c)e face value 1 or 5
    """

    return sum(
        dice.count(die) * score if dice.count(die) == 1 or dice.count(die) == 2
        else (dice.count(die) - 3) * score if dice.count(die) > 3
        else 0
        for die, score in {
            1: 100,
            5: 50
...
def score(dice):
    ...
    return score_triple(dice) + score_single(dice)
